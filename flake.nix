{
  ...
  outputs = { self, nixpkgs, llm-agents-nix, nix-bwrapper }: let
    ...
    configDrv = pkgs: pkgs.runCommand "opencode-config" {} ''
      mkdir -p $out/share/opencode-config
      cp -r ${./config}/* $out/share/opencode-config/
    '';

    sandboxedOpenCode = system: let
      pkgs = import nixpkgs {
        inherit system;
    ...
    wrappedPackage = system: let
      pkgs = nixpkgs.legacyPackages.${system};
      config = configDrv pkgs;
      isLinux = builtins.elem system linuxSystems;
      opencode =
        if isLinux
        then sandboxedOpenCode system
        else llm-agents-nix.packages.${system}.opencode;
    in pkgs.symlinkJoin {
